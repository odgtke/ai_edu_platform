...
CREATE TABLE IF NOT EXISTS edu_assessment (
    ...
    FOREIGN KEY (creator_id) REFERENCES edu_user(user_id),
...
CREATE TABLE IF NOT EXISTS edu_user_assessment (
    ...
    FOREIGN KEY (assessment_id) REFERENCES edu_assessment(assessment_id),
...
-- 初始化测试数据
-- 插入测试评估
INSERT IGNORE INTO edu_assessment (assessment_name, assessment_code, subject_id, grade_level, assessment_type, description, total_score, passing_score, duration, question_count, status, creator_id, start_time, end_time) VALUES
('数学能力测试-初级', 'MATH-001', 1, 1, 1, '小学一年级数学基础能力测试', 100.00, 60.00, 60, 20, 1, 1, '2026-03-20 00:00:00', '2026-12-31 23:59:59'),
('语文阅读理解测试', 'CHINESE-001', 2, 2, 2, '小学二年级语文阅读理解能力测试', 100.00, 70.00, 45, 15, 1, 1, '2026-03-20 00:00:00', '2026-12-31 23:59:59'),
('英语词汇掌握测试', 'ENGLISH-001', 3, 3, 4, '小学三年级英语词汇专项测试', 100.00, 65.00, 30, 25, 1, 1, '2026-03-20 00:00:00', '2026-12-31 23:59:59');

-- 插入测试题目（以第一条评估为例）
INSERT IGNORE INTO edu_assessment_question (assessment_id, question_order, question_type, content, options, correct_answer, score, difficulty, tags, analysis) VALUES
